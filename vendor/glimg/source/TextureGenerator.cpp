...
namespace glimg
{
	...
	namespace
	{
		...
		void ThrowIfRGTCNotSupported()
		{
			if(!GLEW_VERSION_3_0)
			{
				if(!(GLEW_ARB_texture_compression_rgtc || GLEW_EXT_texture_compression_rgtc))
		...
		{
			if(!GLEW_VERSION_1_4) //Yes, really. Depth textures are old.
			{
...
namespace glimg
{
	...
	namespace
	{
		...
		void FinalizeTexture(GLuint texture, GLenum texTarget, const detail::ImageSetImpl *pImage)
		{
			...
			int numMipmaps = pImage->GetMipmapCount();
			glTextureParameteriEXT(texture, texTarget, GL_TEXTURE_BASE_LEVEL, 0);
			glTextureParameteriEXT(texture, texTarget, GL_TEXTURE_MAX_LEVEL, numMipmaps - 1);

			//Ensure the texture is texture-complete.
			const ImageFormat &format = pImage->GetFormat();
			if(IsTypeIntegral(format.Type()))
			{
				glTextureParameteriEXT(texture, texTarget, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
				glTextureParameteriEXT(texture, texTarget, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
			}
			else
			{
				glTextureParameteriEXT(texture, texTarget, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
				glTextureParameteriEXT(texture, texTarget, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
			}
		}
		...
		void TexStorage( GLenum texTarget, Dimensions dims, GLuint numMipmaps, GLenum internalFormat )
		{
			...
			{
			case 1:
				glTexStorage1D(texTarget, numMipmaps, internalFormat, dims.width);
				break;
			case 2:
				glTexStorage2D(texTarget, numMipmaps, internalFormat, dims.width, dims.height);
				break;
			case 3:
				glTexStorage3D(texTarget, numMipmaps, internalFormat, dims.width, dims.height, dims.depth);
				break;
			}
		}
			...
			GLenum internalFormat )
		{
			...
			{
			case 1:
				glTextureStorage1DEXT(texture, texTarget, numMipmaps, internalFormat, dims.width);
				break;
			case 2:
				glTextureStorage2DEXT(texture, texTarget, numMipmaps, internalFormat, dims.width, dims.height);
				break;
			case 3:
				glTextureStorage3DEXT(texture, texTarget, numMipmaps, internalFormat,
			...
			const OpenGLPixelTransferParams &upload)
		{
			for(GLuint mipmap = 0; mipmap < numMipmaps; ++mipmap)
			{
				...
				{
				case 1:
					glTexImage1D(texTarget, mipmap, internalFormat, levelDims.width, 0,
						upload.format, upload.type, NULL);
					break;
				case 2:
					glTexImage2D(texTarget, mipmap, internalFormat, levelDims.width, levelDims.height, 0,
						upload.format, upload.type, NULL);
					break;
				case 3:
					glTexImage1D(texTarget, mipmap, internalFormat, levelDims.width, 0,
			...
			GLenum internalFormat, const OpenGLPixelTransferParams &upload)
		{
			for(GLuint mipmap = 0; mipmap < numMipmaps; ++mipmap)
			{
				...
				{
				case 1:
					glTexImage2D(texTarget, mipmap, internalFormat, levelDims.width, arrayCount, 0,
						upload.format, upload.type, NULL);
					break;
				case 2:
					if(texTarget == GL_TEXTURE_CUBE_MAP_ARRAY)
						arrayCount *= 6;

					glTexImage3D(texTarget, mipmap, internalFormat, levelDims.width, levelDims.height, arrayCount,
			...
			GLenum internalFormat, const OpenGLPixelTransferParams &upload)
		{
			...
			{
				Dimensions levelDims = ModifySizeForMipmap(dims, mipmap);
				glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X, mipmap, internalFormat,
					levelDims.width, levelDims.height, 0, upload.format, upload.type, NULL);
				glTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_X, mipmap, internalFormat,
					levelDims.width, levelDims.height, 0, upload.format, upload.type, NULL);
				glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_Y, mipmap, internalFormat,
					levelDims.width, levelDims.height, 0, upload.format, upload.type, NULL);
				glTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_Y, mipmap, internalFormat,
					levelDims.width, levelDims.height, 0, upload.format, upload.type, NULL);
				glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_Z, mipmap, internalFormat,
					levelDims.width, levelDims.height, 0, upload.format, upload.type, NULL);
				glTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_Z, mipmap, internalFormat,
					levelDims.width, levelDims.height, 0, upload.format, upload.type, NULL);
			}
		}
			...
			GLenum internalFormat, const OpenGLPixelTransferParams &upload)
		{
			...
			for(GLuint mipmap = 0; mipmap < numMipmaps; ++mipmap)
			{
				...
				{
				case 1:
					glTextureImage1DEXT(texture, texTarget, mipmap, internalFormat, levelDims.width, 0,
						upload.format, upload.type, NULL);
					break;
				case 2:
					glTextureImage2DEXT(texture, texTarget, mipmap, internalFormat, levelDims.width, levelDims.height, 0,
						upload.format, upload.type, NULL);
					break;
				case 3:
					glTextureImage1DEXT(texture, texTarget, mipmap, internalFormat, levelDims.width, 0,
			...
			GLuint arrayCount, GLenum internalFormat, const OpenGLPixelTransferParams &upload)
		{
			...
			for(GLuint mipmap = 0; mipmap < numMipmaps; ++mipmap)
			{
				...
				{
				case 1:
					glTextureImage2DEXT(texture, texTarget, mipmap, internalFormat, levelDims.width, arrayCount, 0,
						upload.format, upload.type, NULL);
					break;
				case 2:
					glTextureImage3DEXT(texture, texTarget, mipmap, internalFormat, levelDims.width,
			...
			GLenum internalFormat, const OpenGLPixelTransferParams &upload)
		{
			...
			{
				Dimensions levelDims = ModifySizeForMipmap(dims, mipmap);
				glTextureImage2DEXT(texture, GL_TEXTURE_CUBE_MAP_POSITIVE_X, mipmap, internalFormat,
					levelDims.width, levelDims.height, 0, upload.format, upload.type, NULL);
				glTextureImage2DEXT(texture, GL_TEXTURE_CUBE_MAP_NEGATIVE_X, mipmap, internalFormat,
					levelDims.width, levelDims.height, 0, upload.format, upload.type, NULL);
				glTextureImage2DEXT(texture, GL_TEXTURE_CUBE_MAP_POSITIVE_Y, mipmap, internalFormat,
					levelDims.width, levelDims.height, 0, upload.format, upload.type, NULL);
				glTextureImage2DEXT(texture, GL_TEXTURE_CUBE_MAP_NEGATIVE_Y, mipmap, internalFormat,
					levelDims.width, levelDims.height, 0, upload.format, upload.type, NULL);
				glTextureImage2DEXT(texture, GL_TEXTURE_CUBE_MAP_POSITIVE_Z, mipmap, internalFormat,
					levelDims.width, levelDims.height, 0, upload.format, upload.type, NULL);
				glTextureImage2DEXT(texture, GL_TEXTURE_CUBE_MAP_NEGATIVE_Z, mipmap, internalFormat,
					levelDims.width, levelDims.height, 0, upload.format, upload.type, NULL);
			}
		}
			...
			const void *pPixelData, size_t pixelByteSize)
		{
			...
			case 1:
				if(upload.blockByteCount)
					glCompressedTexSubImage1D(texTarget, mipmap, 0,
					dims.width,
					internalFormat, pixelByteSize, pPixelData);
				else
					glTexSubImage1D(texTarget, mipmap, 0,
					dims.width,
					upload.format, upload.type, pPixelData);
				break;
			case 2:
				if(upload.blockByteCount)
					glCompressedTexSubImage2D(texTarget, mipmap, 0, 0,
					dims.width, dims.height,
					internalFormat, pixelByteSize, pPixelData);
				else
					glTexSubImage2D(texTarget, mipmap, 0, 0,
					dims.width, dims.height,
					upload.format, upload.type, pPixelData);
				break;
			case 3:
				if(upload.blockByteCount)
					glCompressedTexSubImage3D(texTarget, mipmap, 0, 0, 0,
					dims.width, dims.height, dims.depth,
					internalFormat, pixelByteSize, pPixelData);
				else
					glTexSubImage3D(texTarget, mipmap, 0, 0, 0,
					dims.width, dims.height, dims.depth,
					upload.format, upload.type, pPixelData);
				break;
			}
			...
			const void *pPixelData, size_t pixelByteSize)
		{
			...
			case 1:
				if(upload.blockByteCount)
					glCompressedTextureSubImage1DEXT(texture, texTarget, mipmap, 0,
					dims.width,
					internalFormat, pixelByteSize, pPixelData);
				else
					glTextureSubImage1DEXT(texture, texTarget, mipmap, 0,
					dims.width,
					upload.format, upload.type, pPixelData);
				break;
			case 2:
				if(upload.blockByteCount)
					glCompressedTextureSubImage2DEXT(texture, texTarget, mipmap, 0, 0,
					dims.width, dims.height,
					internalFormat, pixelByteSize, pPixelData);
				else
					glTextureSubImage2DEXT(texture, texTarget, mipmap, 0, 0,
					dims.width, dims.height,
					upload.format, upload.type, pPixelData);
				break;
			case 3:
				if(upload.blockByteCount)
					glCompressedTextureSubImage3DEXT(texture, texTarget, mipmap, 0, 0, 0,
					dims.width, dims.height, dims.depth,
					internalFormat, pixelByteSize, pPixelData);
				else
					glTextureSubImage3DEXT(texture, texTarget, mipmap, 0, 0, 0,
					dims.width, dims.height, dims.depth,
					upload.format, upload.type, pPixelData);
				break;
			}
		...
		public:
			...
			~TextureBinder() {if(texture) glBindTexture(texTarget, 0);}
		private:
			GLuint texture;
			GLenum texTarget;
		};
			...
			unsigned int forceConvertBits, GLuint internalFormat, const OpenGLPixelTransferParams &upload)
		{
			...
			if(!(forceConvertBits & USE_DSA))
			{
				bind.Bind(GL_TEXTURE_1D, textureName);
				textureName = 0;
			}

			const int numMipmaps = pImage->GetMipmapCount();
			TexStorageBase(GL_TEXTURE_1D, forceConvertBits, pImage->GetDimensions(),
			...
			for(int mipmap = 0; mipmap < numMipmaps; mipmap++)
			{
				...
				TexSubImage(textureName, GL_TEXTURE_1D, mipmap, internalFormat, dims, upload,
					pPixelData, pImage->GetImageByteSize(mipmap));
			}

			FinalizeTexture(textureName, GL_TEXTURE_1D, pImage);
		}
			...
			unsigned int forceConvertBits, GLuint internalFormat, const OpenGLPixelTransferParams &upload)
		{
			...
			if(!(forceConvertBits & USE_DSA))
			{
				bind.Bind(GL_TEXTURE_CUBE_MAP, textureName);
				textureName = 0;
			}

			const int numMipmaps = pImage->GetMipmapCount();
			TexStorageCube(GL_TEXTURE_CUBE_MAP, forceConvertBits, pImage->GetDimensions(),
				numMipmaps, internalFormat, upload, textureName);
			...
			for(int mipmap = 0; mipmap < numMipmaps; mipmap++)
			{
				...
				for(int faceIx = 0; faceIx < 6; ++faceIx)
				{
					...
					TexSubImage(textureName, GL_TEXTURE_CUBE_MAP_POSITIVE_X + faceIx,
						mipmap, internalFormat, dims, upload,
						pPixelData, pImage->GetImageByteSize(mipmap));
				}
			}

			FinalizeTexture(textureName, GL_TEXTURE_CUBE_MAP, pImage);
		}
			...
			unsigned int forceConvertBits, GLuint internalFormat, const OpenGLPixelTransferParams &upload)
		{
			...
			if(!(forceConvertBits & USE_DSA))
			{
				bind.Bind(GL_TEXTURE_2D, textureName);
				textureName = 0;
			}

			const int numMipmaps = pImage->GetMipmapCount();
			TexStorageBase(GL_TEXTURE_2D, forceConvertBits, pImage->GetDimensions(),
				numMipmaps, internalFormat, upload, textureName);
			...
			for(int mipmap = 0; mipmap < numMipmaps; mipmap++)
			{
				...
				TexSubImage(textureName, GL_TEXTURE_2D, mipmap, internalFormat, dims, upload,
					pPixelData, pImage->GetImageByteSize(mipmap));
			}

			FinalizeTexture(textureName, GL_TEXTURE_2D, pImage);
		}
			...
			unsigned int forceConvertBits, GLuint internalFormat, const OpenGLPixelTransferParams &upload)
		{
			...
			if(!(forceConvertBits & USE_DSA))
			{
				bind.Bind(GL_TEXTURE_3D, textureName);
				textureName = 0;
			}

			const int numMipmaps = pImage->GetMipmapCount();
			TexStorageBase(GL_TEXTURE_3D, forceConvertBits, pImage->GetDimensions(),
				numMipmaps, internalFormat, upload, textureName);
			...
			for(int mipmap = 0; mipmap < numMipmaps; mipmap++)
			{
				...
				TexSubImage(textureName, GL_TEXTURE_3D, mipmap, internalFormat, dims, upload, pPixelData,
					pImage->GetImageByteSize(mipmap));
			}

			FinalizeTexture(textureName, GL_TEXTURE_3D, pImage);
		}
